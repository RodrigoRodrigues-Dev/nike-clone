<template>
  ...
  >
    <icon
      class="popupItem__close"
      name="material-symbols:cancel"
      style="color: #d0d0d0; width: 30px; height: 30px"
      @click="closePopup"
    />
    <div v-if="isCart" class="popupItem__add">
      <icon
        name="material-symbols:verified-rounded"
        style="color: green; margin-right: 0.5rem; width: 25px; height: 25px"
      />
      <h3>Added to Bag</h3>
    </div>
    <div v-else class="popupItem__fav">
      <icon
        ...
        style="color: #fa8072; margin-right: 0.5rem; width: 25px; height: 25px"
      />
    ...
    <div class="popupItem__btns">
      <RouterLink to="/Cart" v-if="isCart">
        <button class="popupItem__btn popupItem__btn--view">
          View Bag ({{ cartStore.items.length }})
        </button>
      </RouterLink>
      <RouterLink to="/Favorites" v-else>
        <button class="popupItem__btn popupItem__btn--view">
          View Favorite ({{ favoriteStore.items.length }})
        </button>
      </RouterLink>
    </div>
  </div>
