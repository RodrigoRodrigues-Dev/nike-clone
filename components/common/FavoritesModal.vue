      ...
      <div class="favorites-modal__mobile-container">
        ...
        <div class="favorites-modal__info-text-mobile">
          <p class="favorites-modal__info-mobile-subtitle">
            {{ favoriteStore.selectSizeItem.subTitle }}
          </p>
          <p class="favorites-modal__info-mobile-title">
            {{ favoriteStore.selectSizeItem.name }}
          </p>
          <p class="favorites-modal__info-mobile-price">
            ${{ favoriteStore.selectSizeItem.price }}.00
          </p>
        </div>
      </div>
      <div class="favorites-modal__info">
        <div class="favorites-modal__info-text">
          <p class="favorites-modal__info-subtitle">
            {{ favoriteStore.selectSizeItem.subTitle }}
          </p>
          <p class="favorites-modal__info-title">
            {{ favoriteStore.selectSizeItem.name }}
          </p>
          <p class="favorites-modal__info-price">
            ${{ favoriteStore.selectSizeItem.price }}.00
          </p>
        </div>
        <div class="favorites-modal__sizes">
          <p
            class="favorites-modal__sizes-label"
            :class="{ 'favorites-modal__sizes-label--active': productAdd }"
          >
            This product (same size) is already in your bag.
          </p>
          <p
            class="favorites-modal__sizes-title"
            :class="{ 'favorites-modal__sizes-title--no-select': sizeSelect }"
          >
            Select Size
          </p>
          <div
            class="favorites-modal__sizes-buttons"
            :class="{ 'favorites-modal__sizes-buttons--no-select': sizeSelect }"
          >
            <button
              class="favorites-modal__size-button"
              :class="{
                'favorites-modal__size-button--active': activeSize === size
              }"
              v-for="size in favoriteStore.selectSizeItem.sizes"
              :key="size"
          ...
          <button
            ...
            class="favorites-modal__add-button"
          >
            Add to Bag
          </button>
        </div>
