        ...
        <div class="search-results">
            <div class="search-results__container">
                <RouterLink to="/" @click="searchResults.toggleSearchResults">
                    <img class="search-results__logo" src="https://nike-vibe-shop-images.vercel.app/nike-logo.png" alt="Logo da Nike">
                </RouterLink>
                <div class="search-results__search">
                    <div class="search-results__search__icon-container">
                        <button class="search-results__search__button">
                            <icon class="search-results__search__icon" name="clarity:search-line" />
                        </button>
                    </div>
                    <input type="search" value="" class="search-results__search__input" placeholder="Search" autocomplete="off" aria-label="Search Products" inputmode="search" @input="handleInput">
                </div>
                <span class="search-results__cancel" @click="searchResults.toggleSearchResults">cancel</span>
            </div>
            <div v-if="productsSearch.length <= 0" class="search-results__popular">
                ...
                <div class="search-results__popular-content">
                    <div v-for="category in popularCategory" class="search-results__popular-item" @click="captureCategory(category)">
            ...
            <div v-else class="search-results__products">
                <div class="search-results__suggestions">
                    ...
                    <div class="search-results__suggestions-list">
                        <span class="search-results__suggestions-item" v-for="item in suggestionsFiltered" :key="item.id" v-html="highlightMatch(item)">
                        </span>
                    </div>
                </div>
                ...
                <template v-else>
                    <div class="search-results__products-content">
                        <div class="search-results__products-card" v-for="item in productsSearch" @click="goToProduct(item), searchResults.toggleSearchResults()">
                            <img :src="item.images.main" alt="" class="search-results__products-card__image">
                            <p class="search-results__products-card__title">{{ item.productName }}</p>
                            <p class="search-results__products-card__description">{{ item.productDescription }}</p>
                            <p class="search-results__products-card__price">${{ item.priceUSD }}.00</p>
                        </div>
                    </div>
                </template>
            </div>
        </div>
        <div class="modal-overlay" @click="searchResults.toggleSearchResults">
        </div>
    </template>
</template>
...
const isLoading = computed(() => searchResults.valueInput !== searchResults.debouncedValueInput);

const suggestionsFiltered = computed(() => {
    return suggestions
        .filter((item) => item.toLowerCase().includes(searchResults.valueInput.toLowerCase()))
        .slice(0, 5);
});
...
.search-results {
    ...
    &__lds-ring div {
        ...
        border-radius: 50%;
        animation: search-results__lds-ring 1.2s cubic-bezier(0.5, 0, 0.5, 1) infinite;
